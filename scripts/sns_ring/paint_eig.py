...
# Eigenpainting parameters (ignored if eig=0).
parser.add_argument("--eig", type=int, default=0)
parser.add_argument("--eig-sc", type=int, default=0)
parser.add_argument("--j1", type=float, default=0.0)
parser.add_argument("--j2", type=float, default=30.0)
...
# Set injection coordinates at time t=0.
...
# Set injection coordinates at time t=1.
...
# Use the lattice eigenvectors to set the injection coordinates at t=1.

def eigenvectors_to_inj_coords(v1=None, v2=None, J1=0.0, J2=0.0, psi1=0.0, psi2=0.0):
    if v1 is None:
        v1 = np.zeros(4)
    if v2 is None:
        v2 = np.zeros(4)
    ...
    x = np.real(x1 + x2)
    coords = np.zeros(4)
    coords[0] = args.inj_x - x[0]
    coords[2] = args.inj_y - x[2]
    coords[1] = -x[1]
    coords[3] = -x[3]
    return coords
...
J1 = args.j1 * 1.00e-06
J2 = args.j2 * 1.00e-06
psi1 = np.radians(args.psi1)
psi2 = np.radians(args.psi2)

if args.eig:
    v1, _, v2, _ = tmat_params_4d["eigenvectors"].T
    inj_coords_t1 = eigenvectors_to_inj_coords(v1=v1, v2=v2, J1=J1, J2=J2, psi1=psi1, psi2=psi2)
...
if args.eig_sc:
    ...
    envelope = DanilovEnvelope22(
        eps_l=15.00e-06,  # ?
        mode=2,  # how to chose?
        eps_x_frac=0.5,  # starting piont
        mass=args.mass,
        kin_energy=args.energy,
        length=(ring.getLength() * args.inj_dist_length),  # estimate
        intensity=(minipulse_intensity * args.inj_turns),
    )
    envelope.set_twiss_2d(alpha_x=0.0, beta_x=10.0, alpha_y=0.0, beta_y=10.0, eps_x_frac=0.5)
    ...
    inj_coords_t1 = envelope.get_particle_coordinates(psi=args.psi1)
        

# Turn off fringe fields for this optimization.
ring.set_fringe_fields(False)
...
print("Target:")
print("x  = {:0.3f} [mm]".format(1000.0 * inj_coords_t0[0]))
print("y  = {:0.3f} [mm]".format(1000.0 * inj_coords_t0[2]))
print("xp = {:0.3f} [mrad]".format(1000.0 * inj_coords_t0[1]))
print("yp = {:0.3f} [mrad]".format(1000.0 * inj_coords_t0[3]))
kicker_angles_t0 = ric.get_kicker_angles()
if not args.noopt:
    kicker_angles_t0 = ric.set_inj_coords(inj_coords_t0, **opt_kws)

print("Injection coords:")
print("Goal:", inj_coords_t0)
ric.print_inj_coords()
traj = ric.get_trajectory()
if _mpi_rank == 0:
...
print("Target:")
print("x  = {:0.3f} [mm]".format(1000.0 * inj_coords_t1[0]))
print("y  = {:0.3f} [mm]".format(1000.0 * inj_coords_t1[2]))
print("xp = {:0.3f} [mrad]".format(1000.0 * inj_coords_t1[1]))
print("yp = {:0.3f} [mrad]".format(1000.0 * inj_coords_t1[3]))
...
if not args.setup:
    monitor = pyorbit_sim.ring.Monitor(
        filename=(man.get_filename("history.dat") if args.save else None), verbose=True
    )
    ...
    def active(turn, freq):
        return freq > 0 and any(
            [
                turn % freq == 0,
                turn == args.inj_turns,
                turn == n_turns,
    ...
    for turn in range(1, n_turns + 1):
        ...
        if args.save and active(turn, args.write_bunch_freq):
            filename = man.get_filename("bunch_{:05.0f}.dat".format(turn))
            bunch.dumpBunch(filename)

        if _mpi_rank == 0 and args.save and active(turn, args.small_freq):
